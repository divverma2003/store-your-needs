...
const FeaturedProducts = (props) => {
  ...
  const [itemsPerPage, setItemsPerPage] = useState(4); // Number of products to show per page
  const { addToCart } = useCartStore();
  const { user } = useUserStore();
  // Handle user resizing their window
  useEffect(() => {
    const handleResize = () => {
  ...
  // Disable buttons if at start or end of product list
  const isStartDisabled = currentIndex === 0;
  const isEndDisabled =
    currentIndex >= props.featuredProducts.length - itemsPerPage;
  return (
    <div className="py-12">
